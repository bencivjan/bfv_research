...
Ciphertext server_compute(vector<Plaintext>& data, vector<Ciphertext>& client_array, size_t len, Evaluator* evaluator);
...
int main() {
    ...
    // t
    uint64_t plain_mod = 524288;//131072;//8192;//1024;
    ...
    for (uint64_t i = 0; i < len; i++) {
        ...
        // encryptor.encrypt_symmetric(i_plain, data[i]);
        data[i] = i_plain;
    }
    t = clock() - start;
    ...
    start = clock();
    Ciphertext server_val = server_compute(data, request, len, &evaluator);
    t = clock() - start;
